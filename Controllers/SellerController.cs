    ...
    public class SellerController : Controller
    {
        ...
        public async Task<IActionResult> EditProperty(int id)
        {
            ...
            var property = await _context.Properties
                .Include(p => p.Images) // IMPORTANT: Include existing images
                .FirstOrDefaultAsync(p => p.PropertyId == id && p.SellerId == sellerId.Value);
            ...
            var model = new PropertyViewModel
            {
                ...
                InitialDeposit = property.InitialDeposit,
                Landmark = property.Landmark,
                ExistingImages = property.Images.ToList() // Pass existing images to the view
            };
            return View(model);
        }

        // In SellerController.cs, REPLACE the [HttpPost] EditProperty method with this:
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditProperty(PropertyViewModel model)
        {
            ...
            var propertyToUpdate = await _context.Properties
                .Include(p => p.Images)
                .FirstOrDefaultAsync(p => p.PropertyId == model.PropertyId && p.SellerId == sellerId.Value);

            if (propertyToUpdate == null) return NotFound();

            // Check if adding new images would exceed the max limit
            if (model.NewImages != null && (propertyToUpdate.Images.Count + model.NewImages.Count > 6))
            {
                ModelState.AddModelError("NewImages", "You can upload a maximum of 6 images per property.");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                propertyToUpdate.InitialDeposit = model.InitialDeposit;
                propertyToUpdate.Landmark = model.Landmark;
                propertyToUpdate.IsActive = false; // Always requires re-verification after an edit
                propertyToUpdate.DeactivationReason = null;
                ...
                if (model.NewImages != null)
                {
                    foreach (var file in model.NewImages)
                    {
                        if (file.Length > 0)
                        {
                            ...
                            var image = new Image
                            {
                                PropertyId = propertyToUpdate.PropertyId,
                                ImageData = ms.ToArray()
                            };
                            _context.Images.Add(image);
                        }
            ...
            // If validation fails, repopulate existing images and return to the view
            model.ExistingImages = propertyToUpdate.Images.ToList();
            return View(model);
        }
